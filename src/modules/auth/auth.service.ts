...
export class AuthService {
  ...
  async login(loginDto: LoginDto): Promise<Record<string, any>> {
    ...
    const userDetails = await this.userservice.findRawOne(loginDto.email);
    ...
    const isValid = bcryptjs.compareSync(loginDto.password, userDetails.password);
    if (isValid) {
      // Generate JWT token
      return {
        status: 200,
        msg: {
          ...
          access_token: this.jwtService.sign({ email: loginDto.email }),
        },
      };
    } else {
